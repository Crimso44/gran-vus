...
uses
  ...
  Db, dxTL, Vcl.Mask, dxTLStr, dxCntner, dxEditor, dxExEdtr, dxGrFCmn, dxDBEdtr
...
type
  ...
  private
    ...
    // !!! Crimso
    FOnGetFieldText: TColumnGetText;
    FOnSetFieldText: TColumnGetText;
    procedure WriteOnGetFieldText(value: TColumnGetText);
    procedure WriteOnSetFieldText(value: TColumnGetText);
  ...
  published
    ...
    // !!! Crimso
    property OnGetFieldText: TColumnGetText read FOnGetFieldText write WriteOnGetFieldText;
    property OnSetFieldText: TColumnGetText read FOnSetFieldText write WriteOnSetFieldText;
  end;
  ...
  protected
    ...
    // Selected
    function CompareBkm(const Item1, Item2: TBookmark): Integer;
    function FindBkm(const Item: TBookmark; var Index: Integer): Boolean;
    ...
    function GetSelectedRow(Index: Integer): TBookmark;
    ...
    property SelectedRows[Index: Integer]: TBookmark read GetSelectedRow;
...
const
  varFieldType: array[TFieldType] of Integer = (
    ...
    varString,         { ftOraTimeStamp }
    varString,         { ftOraInterval }
    varString,         { ftLongWord }
    varString,         { ftShortint }
    varString,         { ftByte }
    varString,         { ftExtended }
    varString,         { ftConnection }
    varString,         { ftParams }
    varString,         { ftStream }
    varString,         { ftTimeStampOffset }
    varString,         { ftObject }
    varString          { ftSingle }
  );
begin
...
begin
  if Assigned(Field) then
  begin
    AText := TdxInplaceTreeListEdit(AInplaceEdit).Text;

    // !!! Crimso
    if Assigned(FOnSetFieldText) then FOnSetFieldText(Self, ANode, AText);
...
begin
  ...
  begin
    ...
    // !!! Crimso
    if Assigned(FOnGetFieldText) then FOnGetFieldText(Self, ANode, AText);

    TdxInplaceTreeListEdit(AInplaceEdit).Text := AText;
...
// !!! Crimso
procedure TdxDBTreeListColumn.WriteOnGetFieldText(value: TColumnGetText);
begin
  FOnGetFieldText := value;
end;

// !!! Crimso
procedure TdxDBTreeListColumn.WriteOnSetFieldText(value: TColumnGetText);
begin
  FOnSetFieldText := value;
end;
...
function TCustomdxDBTreeListControl.CompareBkm(const Item1, Item2: TBookmark): Integer;
begin
...
function TCustomdxDBTreeListControl.FindBkm(const Item: TBookmark; var Index: Integer): Boolean;
var
  L, H, I, C: Integer;
begin
  ...
  begin
    I := (L + H) shr 1;
    C := CompareBkm(TBookmark(FBkmList[I]), Item);
...
function TCustomdxDBTreeListControl.GetSelectedRow(Index: Integer): TBookmark;
begin
  Result := TBookmark(FBkmList[Index]);
end;
...
begin
  if Assigned(Field) then
  begin
    AText := TdxInplaceDBTreeListMaskEdit(AInplaceEdit).Text;
...
begin
  ...
  begin
    ...
    TdxInplaceDBTreeListMaskEdit(AInplaceEdit).Text := AText;
