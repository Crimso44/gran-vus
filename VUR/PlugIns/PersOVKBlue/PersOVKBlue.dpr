  ...
  old_conn := dmMain.dbMain.ConnectionObject;
    try
      ...
      try
       if cbxOVK.Items.Count > 0 then begin
        ...
        MessageBox(Application.Handle,
          'Нет работников, подлежащих снятию со спецучета по возрасту!',
          'Письма в ОВК о снятии со спецучета по возрасту',MB_OK or MB_ICONWARNING);
         Exit;
       end;
      finally Free;
      end;
      ...
      if not dmMain.OpenData(nOrgID) then begin
        ...
        MessageBox(Application.Handle,
          'Ошибка при обращении к базе данных!',
          'Письма в ОВК о снятии со спецучета по возрасту',MB_OK or MB_ICONERROR);
        Result := -1;
      end else
