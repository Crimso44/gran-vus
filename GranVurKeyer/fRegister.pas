...
var
  ...
  mc: TMemCert;
begin
  btnLicenceDel.Enabled := memKeys.Count > 0;
  fDemo := true;
  if memKeys.Count > 0 then begin
//check demo version
    //k := memKeys.RecNo;
    //memKeys.DisableControls;
    try
      for k := 0 to memKeys.Count - 1 do begin
        mc := TMemCert(memKeys.Items[k]);
        if (mc.ValidityDate>Now) and
           (mc.LicenceCount>0) then
        begin
          fDemo := false;
          break;
        end;
    ...
    repeat
      if F.Attr and faDirectory=0 then begin
        ...
        if ValidateStr=string(ValidateCalc) then begin
          ...
          then begin
            mc := TMemCert.Create;
            mc.LicenceNumber := Certificate.LicenceNumber;
            mc.LicenceCount := Certificate.LicenceCount;
            mc.ValidityDate := Certificate.ValidityDate;
            mc.certificate_key :=
...
var k: Integer;
    ...
    s: string;
begin
  memKeys := TObjectList.Create(true);
//load info
...
begin
  CertificateBits := TBits.Create;
  Certificate.ProductCode := StrToInt('$'+StringReplace(lbProductCode.Caption,'-',EmptyStr,[rfReplaceAll]));
  Certificate.LicenceNumber := 1234;
  Certificate.LicenceCount := 44;
  Certificate.ValidityDate := EncodeDate(2117,12,31);
  Certificate.HardwareKey := GetHardwareKey;
  Certificate.ControlKey := Cardinal(StrToInt('$'+StringReplace(RightStr(CodeTableConvert(edRequestCode.Text, 1),9),'-','',[rfReplaceAll])));
  CertificateToBits(Certificate, CertificateBits);
  MixBits(CertificateBits,GetHardwareKey,false);
  ...
  fileName := ExtractFilePath(Application.ExeName)+'\RegInfo\'+Copy(CertificateStr, 1, 25)+'.key';
