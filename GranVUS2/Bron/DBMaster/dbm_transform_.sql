...
select
	...
	1,
	0
from _ORG O JOIN ORG OO ON OO.EXT_ID = O.ORGID AND OO.TERR_ID = @TERR_ID
...
SELECT
	...
	OS.NUMB,
	OS.DD
FROM 	_OSN OS left join VV3 on OS. KOD1 = VV3.KOD left join VV4 on OS.KOD2 = VV4.KOD
	join _ORG on OS.OKPO = _ORG.OKPO_ join ORG O ON O.EXT_ID = _ORG.ORGID AND O.TERR_ID = @TERR_ID
if @@ERROR != 0
begin
	goto end_err
end
...
SELECT
	...
	P.NUMB_P,
	P.RAZD_P
FROM 	_PER P left join VV4 on P.KOD2 = VV4.KOD
	join _ORG on P.OKPO = _ORG.OKPO_ join ORG O ON O.EXT_ID = _ORG.ORGID AND O.TERR_ID = @TERR_ID
if @@ERROR != 0
begin
	goto end_err
end
...
SELECT	DISTINCT
	...
	NULL
FROM	_O_S_V S
	join _ORG OO on S.OKPO = OO.OKPO_ join ORG O on OO.ORGID = O.EXT_ID AND O.TERR_ID = @TERR_ID
if @@ERROR != 0
begin
	goto end_err
end
...
FROM	KCPROF C cross join FORM6HDR F
	join ORG O on F.ORGID = O.ORGID join _ORG OO on O.EXT_ID = OO.ORGID AND O.TERR_ID = @TERR_ID
if @@ERROR != 0
begin
	goto end_err
end
...
from	_O_S_V S join _SPEC ON S.KOD_SPEC = _SPEC.KOD
	join _ORG OO on S.OKPO = OO.OKPO_ join ORG O on OO.ORGID = O.EXT_ID AND O.TERR_ID = @TERR_ID
	join FORM6HDR F on F.ORGID = O.ORGID
where	FORM6.CPROF_ID = _SPEC.CPROF_ID
...
from	(select F.* from FORM6 F join FORM6HDR FH on F.F6_ID = FH.F6_ID
	join ORG O on FH.ORGID = O.ORGID join _ORG OO on O.EXT_ID = OO.ORGID and O.TERR_ID = @TERR_ID
