...
begin
  ...
  try
    ...
    on E: Exception do begin
      ...
      with qryMaster do begin Close; Open; Last; end;
      with qryDetail do begin Close; Open; end;
      ShowErr('Ошибка очистка журнала событий! '#13+E.ClassName+':'+E.Message);
      Exit;
    end;
  end;
...
begin
  aClear.Enabled := GetLastUserID=1;
  aWord.Enabled  := dmMain.rPrint;
  aSaveTo.Enabled := dmMain.rPrint;
  qryMaster.Open;
  with TIniFile.Create('frmdata.ini') do
  try
...
procedure TfmEventList.aSaveToExecute(Sender: TObject);
var Ext: string;
begin
  SaveDialog.FileName:= '';
  if SaveDialog.Execute then begin
    Ext:= AnsiUpperCase(ExtractFileExt(SaveDialog.FileName));
    if Ext = '.XLS' then gridMaster.SaveToXLS(SaveDialog.FileName, True)
    else if (Ext = '.HTM') or (Ext = '.HTML') then gridMaster.SaveToHTML(SaveDialog.FileName, True)
    else if Ext = '.TXT' then gridMaster.SaveToTEXT(SaveDialog.FileName, True, #9, '', '');
  end;
