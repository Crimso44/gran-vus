...
uses
  Windows, Messages, Classes, Vcl.Controls, Vcl.Graphics, Vcl.Forms, dxCntner;
...
begin
  ...
  begin
    ...
    begin
      ...
      S := FormatSettings.LongMonthNames[Month] + ' ' + IntToStr(Year);
...
begin
  begin
    ...
    Result.Bottom := Result.Top + FHeaderHeight;
  end;
end;
...
begin
  begin
    Result.Left := 0;
    Result.Top := FHeaderHeight + Byte(not FFlat);
    Result.Right := ClientWidth;
    Result.Bottom := Result.Top + FDaysOfWeekHeight + 6 * FRowHeight + 1;
  end;
end;
...
begin
  ...
  begin
    Inc(Result.Left, FColWidth);
    Dec(Result.Right, FColWidth + Byte(not FFlat));
    Result.Bottom := Result.Top - Byte(not FFlat) - 1;
    Result.Top := Result.Bottom - (FHeaderHeight - 2);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Inc(R.Left, FSideWidth);
      Dec(R.Right, FSideWidth);
      R.Bottom := R.Top + 6 * FRowHeight;
  ...
  begin
    ...
    begin  // show month's list box
      ...
      begin
        ...
        R.Top := (R.Top + R.Bottom) div 2 - Height div 2;
...
var
  ...
  begin
    with Canvas do
    begin
      if LeftArrow then X := R.Left - 1
      else X := R.Right;
      Sign := 2 * Byte(LeftArrow) - 1;
      P[1] := Point(X + Sign * (FSideWidth - 1), (R.Top + R.Bottom - ArrowHeight) div 2);
  ...
  begin
    ...
    begin
      R := GetInternalRect;
      ExcludeRect(Rect(R.Left + FSideWidth, R.Top, R.Right - FSideWidth, R.Bottom - 1));
      // draw header's frame
      TextR := GetHeaderRect;
      ArrowHeight := (TextR.Bottom - TextR.Top) div 2;
      ...
      if not FFlat then
        begin
          MoveToEx(Handle, TextR.Left, TextR.Bottom, nil);
          Windows.LineTo(Handle, TextR.Right, TextR.Bottom);
          if Col = ColCount - 1 then
          begin
            MoveToEx(Handle, TextR.Right - 1, TextR.Top, nil);
            Windows.LineTo(Handle, TextR.Right - 1, TextR.Bottom);
            Dec(TextR.Right);
          end;
        end;
      ...
      if Col < ColCount - 1 then
        begin
          SideR := Rect(TextR.Right - 1, TextR.Top + 2, TextR.Right + 1, TextR.Bottom - 2);
          DrawEdge(Handle, SideR, EDGE_ETCHED, BF_LEFT);
          // ??? with SideR do
          begin
            Windows.FillRect(Handle, Rect(TextR.Left, TextR.Top - 1, TextR.Right, TextR.Top), Brush.Handle);
            Windows.FillRect(Handle, Rect(TextR.Left, TextR.Bottom, TextR.Right, TextR.Bottom + 1), Brush.Handle);
          end;
        end;
      ...
      S := FormatSettings.LongMonthNames[Month] + ' ' + IntToStr(Year);
      GetTextExtentPoint32(Handle, PChar(S), Length(S), XSize);
      SetTextColor(Handle, GetSysColor(COLOR_BTNTEXT));
      Brush.Color := clBtnFace;
      ExtTextOut(Handle, (TextR.Left + TextR.Right - XSize.cX) div 2, (TextR.Top + TextR.Bottom - XSize.cY) div 2,
          ETO_CLIPPED or ETO_OPAQUE, @TextR, PChar(S), Length(S), nil);
      // write first letters of day's names
      Brush.Color := clWindow;
      begin
        TextR.Left := R.Left + FSideWidth;
        TextR.Right := R.Right - FSideWidth;
        TextR.Top := R.Top;
        TextR.Bottom := Top + FDaysOfWeekHeight - 2;
        Windows.FillRect(Handle, Rect(TextR.Left - 8, TextR.Top, TextR.Left, TextR.Bottom + 2), Brush.Handle);
        Windows.FillRect(Handle, Rect(TextR.Right, TextR.Top, TextR.Right + 8, TextR.Bottom + 2), Brush.Handle);
        Pen.Color := clBtnShadow;
        MoveToEx(Handle, TextR.Left, TextR.Bottom, nil);
        Windows.LineTo(Handle, TextR.Right, TextR.Bottom);
        Pen.Color := clWindow;
        MoveToEx(Handle, TextR.Left, TextR.Bottom + 1, nil);
        Windows.LineTo(Handle, TextR.Right, TextR.Bottom + 1);
        TextR.Right := TextR.Left;
      end;
      for I := 0 to 6 do
      begin
        begin
          TextR.Left := TextR.Right;
          TextR.Right := TextR.Left + FColWidth;
        end;
        J := StartOfWeek + 1 + I;
        if J > 7 then Dec(J, 7);
        S := {$IFDEF DELPHI3}WideString{$ENDIF}(FormatSettings.ShortDayNames[J])[1];
        GetTextExtentPoint32(Handle, PChar(S), Length(S), XSize);
        ExtTextOut(Handle, TextR.Right - 3 - XSize.cX, (TextR.Top + TextR.Bottom - XSize.cY) div 2,
      ...
      for I := 0 to 6 do
        ...
        begin
          begin
            TextR.Left := R.Left + FSideWidth + I * FColWidth;
            TextR.Top := R.Top + FDaysOfWeekHeight + J * FRowHeight;
            TextR.Right := TextR.Left + FColWidth;
            TextR.Bottom := TextR.Top + FRowHeight;
          end;
          ...
          begin
            Pen.Color := clMaroon;
            Polyline([Point(TextR.Left, TextR.Bottom - 1), Point(TextR.Left, TextR.Top),
                Point(TextR.Right - 1, TextR.Top), Point(TextR.Right - 1, TextR.Bottom - 1),
                Point(TextR.Left, TextR.Bottom - 1)]);
            InflateRect(TextR, -1, -1);
          end;
          ...
          GetTextExtentPoint32(Handle, PChar(S), Length(S), XSize);
          ExtTextOut(Handle,
              SideR.Right - 3 - XSize.cX, (SideR.Top + SideR.Bottom - XSize.cY) div 2,
              ETO_OPAQUE, @TextR, PChar(S), Length(S), nil);
        end;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        Pen.Color := clBtnText;
        MoveToEx(Handle, R.Left, R.Bottom - 1, nil);
        Windows.LineTo(Handle, R.Right - 1, R.Bottom - 1);
        Windows.LineTo(Handle, R.Right - 1, R.Top);
        Pen.Color := clBtnFace;
        Windows.LineTo(Handle, R.Left, R.Top);
        Windows.LineTo(Handle, R.Left, R.Bottom - 1);
        Pen.Color := clBtnShadow;
        MoveToEx(Handle, R.Left + 1, R.Bottom - 2, nil);
        Windows.LineTo(Handle, R.Right - 2, R.Bottom - 2);
        Windows.LineTo(Handle, R.Right - 2, R.Top + 1);
        Pen.Color := clBtnHighlight;
        Windows.MoveToEx(Handle, R.Left + 1, R.Bottom - 3, nil);
        Windows.LineTo(Handle, R.Left + 1, R.Top + 1);
        Windows.LineTo(Handle, R.Right - 2, R.Top + 1);
        SetPixel(Handle, R.Right - 2, R.Top + 1, GetSysColor(COLOR_BTNFACE));
      end;
      // draw button's caption
      InflateRect(R, -2, -2);
      Brush.Color := clBtnFace;
      SetTextColor(Handle, GetSysColor(COLOR_BTNTEXT));
      GetTextExtentPoint32(Handle, PChar(ACaption), Length(ACaption), XSize);
      ExtTextOut(Handle, (R.Left + R.Right - XSize.cX) div 2 + Byte(Pressed),
        (R.Top + R.Bottom - XSize.cY) div 2 + Byte(Pressed), ETO_CLIPPED or ETO_OPAQUE, @R,
