...
begin
  ...
  try
    ...
    SaveEvent(dmMain.dbMain, evs_Form6_Generate, '',
...
begin
  ...
  then
    Exit;
  dmMain.Form6Query.Locate('COL_B','24',[]);
...
begin
  ...
  with dmMain.Form6Query do begin
    ...
    if n<>0 then begin
      ...
      if RMessageDlg(
                  'Сумма значений колонок 3 - 4 не равна значению колонки 2!'#13'Отклонение '+IntToStrSign(-n)+'.'#13'Продолжить проверку?', mtWarning, [mbYes, mbNo], 0) = mrNo
    ...
    if n<>0 then begin
      ...
      if RMessageDlg(
                  'Сумма значений колонок 7 - 8 не равна значению колонки 6!'#13'Отклонение '+IntToStrSign(-n)+'.'#13'Продолжить проверку?', mtWarning, [mbYes, mbNo], 0) = mrNo
                    then begin Result := mrNo; Exit; end;
    end;
    // 4 >= 5
    ...
    if n>0 then begin
      ...
      if RMessageDlg(
                  'Значение колонки 5 больше значения колонки 4!'#13'Отклонение '+IntToStrSign(n)+'.'#13'Продолжить проверку?', mtWarning, [mbYes, mbNo], 0) = mrNo
                    then begin Result := mrNo; Exit; end;
    end;
    //1 >= 2+11
    ...
    if n<0 then begin
      ...
      if RMessageDlg(
                  'Сумма значений колонок 2 и 11 больше значения колонки 1!'#13'Отклонение '+IntToStrSign(-n)+'.'#13'Продолжить проверку?', mtWarning, [mbYes, mbNo], 0) = mrNo
                    then begin Result := mrNo; Exit; end;
    end;
    // 2 = 6+9+10
    n := FieldByName('COL_2').AsInteger -
    ...
    if n<>0 then begin
      dxDBGrid.FocusedField:= FieldByName('COL_2');
      if RMessageDlg(
                  'Сумма значений колонок 6, 9 и 10 не равно значению колонки 2!'#13'Отклонение '+IntToStrSign(-n)+'.'#13'Продолжить проверку?', mtWarning, [mbYes, mbNo], 0) = mrNo
                    then begin Result := mrNo; Exit; end;
    end;
    // 3 >= 7
    ...
    if n<0 then begin
      ...
      if RMessageDlg(
                  'Значение колонки 3 меньше значения колонки 7!'#13'Отклонение '+IntToStrSign(n)+'.'#13'Продолжить проверку?', mtWarning, [mbYes, mbNo], 0) = mrNo
                    then begin Result := mrNo; Exit; end;
    end;
    // 4 >= 8
    ...
    if n<0 then begin
      ...
      if RMessageDlg(
                  'Значение колонки 4 меньше значения колонки 8!'#13'Отклонение '+IntToStrSign(n)+'.'#13'Продолжить проверку?', mtWarning, [mbYes, mbNo], 0) = mrNo
                    then begin Result := mrNo; Exit; end;
    end;
  end;
end;
  ...
  with dmMain.Form6Query do begin
    ...
    if dmMain.NoBronMode then begin
      ...
      case CPROF_ID of
        ...
        200         : begin Expr := '210+221+231+241+251+271+281+291+295'; Oper := oEq; end;
        //13 = 14 +15 +16 + 17 + 18 + 19
        400         : begin Expr := '410+440+450+460+470+480';             Oper := oEq; end;
        //22 = 1 + 2 + 12 + 13
        1000        : begin Expr := '100+200+300+400';                     Oper := oEq; end;
        //495,496,500 : begin Expr := '1000';                                Oper := oLeEq; end;
        1500        : begin Expr := '0';                                   Oper := oEq; end;
        else begin
          Expr := FieldByName('EXPRESSION').AsString;
          Oper := oLeEq;
...
begin
  with dmMain.Form6Query do begin
    ...
    try
      bm:= GetBookmark;
      dmMain.Form6HdrQuery.Edit;
      if Locate('CPROF_ID', 2000, []) then begin
...
begin
  Result := ANode.Strings[COL_BColumn.Index]<>EmptyStr;
  if Result then
    if miRecalcCols.Checked then
      Result := (AColumn.FieldName<>'COL_2') and
                (AColumn.FieldName<>'COL_6') and
                (AColumn.FieldName<>'COL_9');
  if Result then
  ...
  if Result then begin
    if dmMain.NoBronMode then
      Result := ANode.Strings[COL_BColumn.Index] = '23'
    else
      Result := ANode.Strings[COL_BColumn.Index] <> '23';
  end;
...
begin
  Allow := Node.Strings[COL_BColumn.Index]<>EmptyStr;
  if Allow then
