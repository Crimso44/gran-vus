...
uses
  ...
  cxGraphics, cxScrollBar, cxClasses, cxLookAndFeels, cxLookAndFeelPainters, dxCore7, dxMessages7;
...
var
  ...
  FUser32DLL: HMODULE;
  dxProcSetMenuInfo: function (hMenu: HMENU; const lpcmi: TMenuInfo): BOOL; stdcall;
...
begin
  Result := LoadMenu(FUser32DLL, MAKEINTRESOURCE(MenuTypes[AMenuType]));
  if Result = 0 then
    Exit;
  if Assigned(dxProcSetMenuInfo) then
  begin
    ...
    dxProcSetMenuInfo(Result, AMenuInfo);
  end;
...
initialization
  ...
  dxProcSetMenuInfo := GetProcAddress(FUser32DLL, 'SetMenuInfo');
