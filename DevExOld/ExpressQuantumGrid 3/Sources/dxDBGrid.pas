...
uses
  ...
  Db, dxTL, Vcl.Mask, dxTLStr, dxCntner, dxEditor, dxDBCtrl,
...
type
  ...
  private
    ...
    function RestoreBookmark(ANode: TdxTreeListNode; const ABookmark: TBookmark; ABackward: Boolean): Boolean;
...
uses Vcl.Menus, Vcl.Clipbrd, Vcl.Consts, Registry, IniFiles
    ...
    try
      ...
      begin
        ...
        try
          ...
          begin
            if CompareBkm(TBookmark(BkmList[I]), Current) = 0 then
              Current := []; // Check Current
            Bookmark := TBookmark(BkmList[I]);
            Delete;
          end
          else
          begin
...
begin
  ...
  begin
    ...
    if ASelected then
    begin
      BkmList.Insert(Index, String(Current));
      ...
      try
        ...
        begin
          ...
          repeat
            ...
            begin
              BkmList.Insert(0, String(Current));
              Prior;
            end
            else
            begin
        ...
        begin
          ...
          begin
            ...
            repeat
              ...
              if ADirectionUp then
                BkmList.Insert(0, String(Current))
              else BkmList.Add(String(Current));
...
var
  ...
  AStartBookmark, AEndBookmark: TBookmark;
  ASetStartBookmark, ASelEndBookmark: TBookmark;
    ...
    begin
      ...
      begin
        if CanNodeSelected(Node) then
          BkmList.AddObject(String(B), Node);
      end
      else
      begin
        ...
        begin
          if ABackward then
            BkmList.InsertObject(0, String(B), Node)
          else BkmList.AddObject(String(B), Node);
        end;
      end;
    end;
          ...
          case Mode of
            ...
            lmCurrent, lmAllRecords: begin
              ...
              begin
                ...
                if FSaveBkmList.Count > 0 then
                begin
                  AStartBookmark := TBookmark(FSaveBkmList[0]);
                  AEndBookmark := TBookmark(FSaveBkmList[FSaveBkmList.Count - 1]);
                ...
                begin
                  ...
                  // Correct Ranges
                  if Length(AStartBookmark) = 0 then AStartBookmark := ASetStartBookmark;
                  if Length(AEndBookmark) = 0 then AEndBookmark := ASelEndBookmark;
                ...
                while (RecNumber >= 0) or
                  ((Length(AStartBookmark) > 0) and (CompareBkm(Bookmark, AStartBookmark) >= 0)) do
                begin
                ...
                while not EOF and ((RecNumber < ARecRangeNumber) or
                  ((Length(AEndBookmark) > 0) and (CompareBkm(Bookmark, AEndBookmark) <= 0))) do
                begin
                  if (Length(AEndBookmark) > 0) and (RecNumber >= ARecRangeNumber) and
...
procedure TCustomdxDBGrid.LoadGroupList(FNodes: TList);
  ...
  begin
    ...
    begin
      FindBkm(B, Index);
      BkmList.InsertObject(Index, String(B), Node);
    end;
  end;
...
begin
  ...
  begin
    if CompareBkm(TBookmark(BkmList[I]), TBookmark(BkmList[I - 1])) = 0 then
      BkmList.Delete(I)
    else
    if AClearNodes then
...
function TCustomdxDBGrid.RestoreBookmark(ANode: TdxTreeListNode; const ABookmark: TBookmark; ABackward: Boolean): Boolean;
var
  L, H, I, C: Integer;
begin
  Result := False;
  ...
  begin
    I := (L + H) shr 1;
    C := CompareBkm(TBookmark(FSaveBkmList[I]), ABookmark);
    if C < 0 then L := I + 1
    else
    begin
      H := I - 1;
      ...
      begin
        if ABackward then
        begin
          BkmList.Insert(0, String(ABookmark));
          BkmList.Objects[0] := ANode;
        end
        else
        begin
          BkmList.Add(String(ABookmark));
