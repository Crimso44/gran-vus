...
const
  sEventObject = 'Письма в ОВК о снятии со спецучета уволенных';
...
begin
  //try
    ...
    qrPerson.Open;
    EkRTF1.ClearVars;
    EkRTF1.CreateVar('TITLE', 'Прошу снять со специального учета граждан, перечисленных в прилагаемой ниже таблице, в связи с их увольнением.');
    EkRTF1.CreateVar('TITLE1', 'СПИСОК ГРАЖДАН, РАБОТАВШИХ И В НАСТОЯЩИЙ МОМЕНТ УВОЛЕННЫХ В');
    EkRTF1.CreateVar('TITLE2', ', ПРЕДСТАВЛЯЕМЫЙ ДЛЯ СНЯТИЯ РАБОТНИКОВ СО СПЕЦИАЛЬНОГО УЧЕТА');
    EkRTF1.CreateVar('YELLOW_HEAD', ', дата увольнения (приказ)');
    FillExecutor;
...
begin
  ...
  EkRTF1.Outfile := GetReportsDir + 'Письма в ОВК о снятии со спецучета уволенных.rtf';
end;
...
begin
  ...
  qrPersonYellow_Data.AsString := ' уволен ' +
    qrPersonOut_Date.AsString +
    '(№' + qrPersonOut_Ord_Numb.AsString + ' от ' + qrPersonOut_Ord_Date.AsString + ')';
