...
const
  sEventObject = 'Численность по видам экономической деятельности 2008';
...
begin
  EkRTF1.Infile := tlPath + 'formokved.rtf';
  EkRTF1.Outfile := repPath + 'Форма по ВЭД.rtf';
  ...
  try
    try
      ...
      with TADOQuery.Create(Self) do try
        ...
        Close;
        SQL.Text := 'SELECT'#10 +
                    '  SUM_RAB_COUNT      = CAST(SUM(RAB_COUNT) AS NUMERIC(15,3))/1000'#10 +
                    ', SUM_ZAP_COUNT      = CAST(SUM(ZAP_COUNT) AS NUMERIC(15,3))/1000'#10 +
                    ', SUM_ZAB_COUNT      = CAST(SUM(ZAB_COUNT) AS NUMERIC(15,3))/1000'#10 +
                    ', SUM_ORG_COUNT      = SUM(ORG_COUNT)'#10 +
                    ', SUM_ORG_BRON_COUNT = SUM(ORG_BRON_COUNT)'#10 +
                    ', SUM_ORG_WU_COUNT   = SUM(ORG_WU_COUNT)'#10 +
                    'FROM FOKVED'#10 +
                    'WHERE SUBJ_ID = ' + IntToStr(ID);
        Open;
        EkRTF1.CreateVar('SUM_RAB_COUNT',      FieldByName('SUM_RAB_COUNT').AsFloat);
        EkRTF1.CreateVar('SUM_ZAP_COUNT',      FieldByName('SUM_ZAP_COUNT').AsFloat);
        EkRTF1.CreateVar('SUM_ZAB_COUNT',      FieldByName('SUM_ZAB_COUNT').AsFloat);
        if FieldByName('SUM_RAB_COUNT').AsFloat>0 then begin
          EkRTF1.CreateVar('SUM_ZAP_PERC',  Trunc((FieldByName('SUM_ZAP_COUNT').AsFloat*100)/
                                                FieldByName('SUM_RAB_COUNT').AsFloat+0.5));
          EkRTF1.CreateVar('SUM_ZAB_PERC',Trunc((FieldByName('SUM_ZAB_COUNT').AsFloat*100)/
                                                FieldByName('SUM_ZAP_COUNT').AsFloat+0.5));
        end
        else begin
          EkRTF1.CreateVar('SUM_ZAP_PERC',     '');
          EkRTF1.CreateVar('SUM_ZAB_PERC',     '');
        end;
        EkRTF1.CreateVar('SUM_ORG_COUNT',      FieldByName('SUM_ORG_COUNT').AsInteger);
        EkRTF1.CreateVar('SUM_ORG_BRON_COUNT', FieldByName('SUM_ORG_BRON_COUNT').AsInteger);
        EkRTF1.CreateVar('SUM_ORG_WU_COUNT',   FieldByName('SUM_ORG_WU_COUNT').AsInteger);
        if FieldByName('SUM_ORG_COUNT').AsInteger>0 then begin
          EkRTF1.CreateVar('SUM_ORG_BRON_PERC',  Trunc((FieldByName('SUM_ORG_BRON_COUNT').AsInteger*100)/
                                                FieldByName('SUM_ORG_COUNT').AsInteger+0.5));
          EkRTF1.CreateVar('SUM_ORG_WU_PERC',Trunc((FieldByName('SUM_ORG_WU_COUNT').AsInteger*100)/
                                                FieldByName('SUM_ORG_COUNT').AsInteger+0.5));
        end
        else begin
          EkRTF1.CreateVar('SUM_ORG_BRON_PERC',     '');
          EkRTF1.CreateVar('SUM_ORG_WU_PERC',       '');
        end;
      finally
        Free;
      end;
      ...
      qrData.Open;
      EkRTF1.CreateVar('MAX_ROW', qrData.RecordCount);
