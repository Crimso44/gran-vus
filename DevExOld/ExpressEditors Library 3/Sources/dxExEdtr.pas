...
uses
  SysUtils, Messages, RichEdit, Windows, Classes, Vcl.Graphics, Vcl.Menus, Vcl.Controls,
  Vcl.Forms, Vcl.StdCtrls, Vcl.Mask, CommCtrl, Vcl.Buttons, Vcl.Clipbrd, Vcl.ImgList, Variants,
  dxCore7, dxCommon7, dxCntner, dxEditor, dxCalc, dxGrDate;
...
uses
  ShellApi, Vcl.ExtDlgs, dxUtils, Vcl.Consts, dxEdStr;
...
begin
  ...
  begin
    ...
    S := Copy(AText, 1, L);
    // !!! Crimso
    if False and (AnsiCompareText(S, sdxDateEditSmartInput[I]) = 0) then
    begin
      case I of
        deiToday:
          ADate := Date;
...
begin
  ...
  try
    ...
    begin
      ...
      if not UseDelphiDateTimeFormats and (FormatSettings.DateSeparator = '.') then
      begin
        C := 0;
...
begin
  ...
  begin
    SetLength(Result, FormatSettings.CurrencyDecimals);
...
var
  ...
  begin
    FillRect(ADC, Rect(X, Y, X + XSize, Y + XSize), GetSysColorBrush(Color));
    FillRect(ADC, Rect(X - XSize * 2, Y, X - XSize * 2 + XSize, Y + XSize), GetSysColorBrush(Color));
    FillRect(ADC, Rect(X + XSize * 2, Y, X + XSize * 2 + XSize, Y + XSize), GetSysColorBrush(Color));
  end;
        ...
        btsSimple:
          begin
            ...
            begin
              ...
              InflateRect(ARect, -1, -1);
              begin
                FillRect(ADC, Rect(ARect.Left, ARect.Top, ARect.Left + 1, ARect.Bottom), Brush);
                Inc(ARect.Left);
              end;
              R := ARect;
              if Kind <> bkGlyph then
...
//              Inc(ARect.Left);
        ...
        btsFlat:
          begin
            if not ATransparent then
              begin
                FillRect(ADC, Rect(ARect.Left, ARect.Top, ARect.Left + 1, ARect.Bottom), GetSysColorBrush(COLOR_BTNFACE));
                Inc(ARect.Left);
              end;
        ...
        bkGlyph:
          begin
            ...
            begin
              with Glyph do
              begin
                BRect := Rect(ARect.Left, ARect.Top, ARect.Left + Width, ARect.Top + Height);
                if BRect.Right < ARect.Right then
                begin
                  if APushed and (BS in [btsFlat, btsSimple]) and Odd(ARect.Right - BRect.Right) then
                    OffsetRect(BRect, 1, 0);
                  OffsetRect(BRect, (ARect.Right - BRect.Right) div 2, 0);
                end;
                if BRect.Bottom < ARect.Bottom then
                  OffsetRect(BRect, 0, (ARect.Bottom - BRect.Bottom) div 2);
              end;
...
begin
  ...
  try
    ...
    begin
      W := ViewInfo.BorderRect.Right - ViewInfo.BorderRect.Left;
      ...
      begin
        Inc(R.Left, ViewInfo.BorderRect.Left);
        R.Right := R.Left + W;
      end;
    end;
  ...
  begin
    ...
    case PopupAlignment of
      ...
      taCenter:
        P := Point((R.Right + R.Left) div 2  - (W div 2), R.Bottom);
    else { taRightJustify }
      P := Point(R.Right - W, R.Bottom);
    end;
    CheckScreenPosition(P, W, H, R.Bottom - R.Top);
  end;
  ...
  begin
    begin
      ...
      try
        ...
        BitBlt(DrawDC, R.Left, R.Top, R.Right - R.Left, R.Bottom - R.Top, DC, 0, 0, SRCCOPY);
      finally
...
begin
  ...
  begin
    begin
      RC.Left := (R.Left + R.Right - ACheckWidth) div 2;
      RC.Top := (R.Top + R.Bottom - ACheckHeight) div 2;
...
//  IsControl := False;
  ...
  begin
    ...
    begin
      ...
      begin
        begin
          R.Left := (ARect.Left + ARect.Right - W) div 2;
          R.Top := (ARect.Top + ARect.Bottom - H) div 2;
          R.Right := R.Left + W;
          R.Bottom := R.Top + H;
        end;
    ...
    begin
      ...
      begin
        ...
        if (0 <= I) and (I < Images.Count) then
          Images.Draw(Canvas, Rect.Left + 1, (Rect.Top + Rect.Bottom - H) div 2, I);
      end
      else W := 0;
      S := Items[Index];
      ...
      begin
        ...
        TextOut(Rect.Left + W + 2 + 1, (Rect.Top + Rect.Bottom - TextHeight(S)) div 2, S);
    end;
end;
...
var
  ...
  begin
    ...
    begin
      P[1] := Point(X, Y + XSize div 2);
      P[2] := Point(X + XSize div 2, Y);
      P[3] := Point(X + XSize - 1, Y + XSize div 2);
    end;
    Pen := SelectObject(ADC, CreatePen(PS_SOLID, 1, GetSysColor(Color)));
    Brush := SelectObject(ADC, GetSysColorBrush(Color));
    Polygon(ADC, P, 3);
  ...
  begin
      ...
      btsSimple:
        begin
          ...
          begin
            ...
            InflateRect(ARect, -1, -1);
            begin
              FillRect(ADC, Rect(ARect.Left, ARect.Top, ARect.Left + 1, ARect.Bottom), ABrush);
              Inc(ARect.Left);
            end;
            R := ARect;
            FillRect(ADC, ARect, Br); // Bk
    ...
    // Content
    begin
      XSize := (ARect.Right - ARect.Left -2{TODO}) div 2; // TODO Height
      if not Odd(XSize) then Inc(XSize);
      X := (ARect.Left + ARect.Right - XSize) div 2;
      Y := (ARect.Top + ARect.Bottom - XSize div 2) div 2 - 1;
      if AEnabled then
...
begin
  ...
  begin
    ...
    begin
      ...
      R := Rect(BR.Right - ButtonWidth, BR.Top, BR.Right, BR.Bottom);
      // Up
      BR := Rect(R.Left, R.Top, R.Right, (R.Top + R.Bottom ) shr 1);
...
begin
  ValidChars := ['+', '-', '0'..'9'];
  if FValueType = vtFloat then ValidChars := ValidChars + [FormatSettings.DecimalSeparator];
...
begin
  if not (UseThousandSeparator and (Key = FormatSettings.ThousandSeparator)) and
    (Key in ['.', ',']) then
...
begin
  ...
  if UseThousandSeparator then
    ValidChars := ValidChars + [FormatSettings.ThousandSeparator];
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              IntersectClipRect(Handle, R.Left + 1, R.Top + 1, R.Right - 1, R.Bottom - 1);
            end
            else
            ...
            if Stretch then
              ...
              begin
                R1 := Rect(TempRect.Left, TempRect.Top, TempRect.Left + Picture.Width, TempRect.Top + Picture.Height);
                if Center then
                begin
                  OffsetRect(R1, (TempRect.Right - TempRect.Left - Picture.Width) div 2, 0);
                  OffsetRect(R1, 0, (TempRect.Bottom - TempRect.Top - Picture.Height) div 2);
                end;
              end;
            ...
            begin
              ...
              begin
                ...
                begin
                  begin
                    Windows.FillRect(Handle, Rect(0, 0, R.Right - R.Left, R.Bottom - R.Top),
                      TdxGraphicEditViewData(AViewData).Brush);
                    OffsetRect(R1, -R.Left, -R.Top);
                  end;
                  StretchDraw(R1, Picture.Graphic);
                end;
                Windows.BitBlt(Handle, R.Left, R.Top, R.Right - R.Left, R.Bottom - R.Top,
            ...
            begin
              StretchDraw(R1, Picture.Graphic);
              ExcludeClipRect(Handle, R1.Left, R1.Top, R1.Right, R1.Bottom);
              Windows.FillRect(Handle, R, TdxGraphicEditViewData(AViewData).Brush)
            end;
...
//          if DrawShadow and (TempDrawBitmap <> nil) then
          if not (Transparent or (not DrawShadow and (TempDrawBitmap = nil))) then
            BitBlt(ADC, ARect.Left, ARect.Top, ARect.Right - ARect.Left, ARect.Bottom - ARect.Top, Handle, 0, 0, SRCCOPY);
        end;
      finally
        TempCanvas.Handle := 0;
        TempCanvas.Unlock;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            begin
              Windows.FillRect(Handle, Rect(0, 0, CR.Right - CR.Left, CR.Bottom - CR.Top),
                Self.Canvas.Brush.Handle);
              OffsetRect(R, CR.Left, CR.Top);
            end;
            StretchDraw(R, DrawPict.Graphic);
          end;
          Windows.BitBlt(Handle, CR.Left, CR.Top, CR.Right - CR.Left, CR.Bottom - CR.Top,
  ...
  begin
    ...
    with FEditPopupMenu.Items do
      ...
      begin
        ...
        Add(NewItem(ToolButtons[ptbCut, 0], Flag and not ReadOnly, Vcl.Menus.ShortCut(Ord('X'), [ssCtrl]), Integer(ptbCut)));
        Add(NewItem(ToolButtons[ptbCopy, 0], Flag, Vcl.Menus.ShortCut(Ord('C'), [ssCtrl]), Integer(ptbCopy)));
        Add(NewItem(ToolButtons[ptbPaste, 0], not ReadOnly and CanPaste, Vcl.Menus.ShortCut(Ord('V'), [ssCtrl]), Integer(ptbPaste)));
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        InflateRect(R, 1, 1);
        SetPixel(DC, R.Left + 1, R.Bottom - 2, GetSysColor(COLOR_BTNFACE));
        SetPixel(DC, R.Right - 2, R.Top + 1, GetSysColor(COLOR_BTNFACE));
      end;
      InflateRect(R, -2, -2);
    end;
...
begin
  ...
  begin
    ...
    if (H <> 0) and (W <> 0) then
    begin
      CR := Rect(R.Right - W, R.Bottom - H, R.Right, R.Bottom);
      if SizingCorner in [coBottomLeft, coBottomRight] then
...
const
  ...
  begin
    ...
    begin
      if PtInRect(Rect(R.Left, R.Top, R.Left + CX, R.Top + CY), P) then
        Result := Windows.HTTOPLEFT
      else
      if PtInRect(Rect(R.Right - CX, R.Top, R.Right, R.Top + CY), P) then
        Result := Windows.HTTOPRIGHT
      else
      if PtInRect(Rect(R.Left, R.Top, R.Right, R.Top + CY), P) then
        Result := Windows.HTTOP
      else
      if PtInRect(Rect(R.Right - CX, R.Bottom - CY, R.Right, R.Bottom), P) then
        Result := Windows.HTBOTTOMRIGHT
      else
      if PtInRect(Rect(R.Right - CX, R.Top, R.Right, R.Bottom), P) then
        Result := Windows.HTRIGHT
      else
      if PtInRect(Rect(R.Left, R.Bottom - CY, R.Left + CX, R.Bottom), P) then
        Result := Windows.HTBOTTOMLEFT
      else
      if PtInRect(Rect(R.Left, R.Bottom - CY, R.Right, R.Bottom), P) then
        Result := Windows.HTBOTTOM
      else
      if PtInRect(Rect(R.Left, R.Top, R.Left + CX, R.Bottom), P) then
...
begin
  ...
  begin
    ...
    ExcludeClipRect(DC, CR.Left, CR.Top, CR.Right, CR.Bottom);
    OffsetRect(CR, FCloseButtonRect.Left, FCloseButtonRect.Top);
    FCloseButtonRect := CR;
