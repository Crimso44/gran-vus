...
uses
  ...
  fFAnaliz in 'fFAnaliz.pas' {fmFSVT},
...
begin
  ...
  try
      ...
      try
        ...
        Application.CreateForm(TfmFSVT, fmFSVT);
        fmFSVT.ID := nSubjID;
        if FPrintOnly then fmFSVT.actPrint.Execute
        else begin
          fmFSVT.actPrint.Visible := False;
          with TADOQuery.Create(nil) do
          try
          ...
          fmFSVT.actGenerate .Enabled := rEdit;
          fmFSVT.actClear    .Enabled := rEdit;
          fmFSVT.actCreate   .Enabled := rEdit;
          if not rEdit then with fmFSVT.dxDBGrid do OptionsBehavior := OptionsBehavior-[edgoEditing];
          fmFSVT.ShowModal;
        end;
      except
