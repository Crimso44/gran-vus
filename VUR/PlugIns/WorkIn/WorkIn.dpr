    ...
    if ShowModal<>mrOk then Result := -3
      ...
      if not dmMain.OpenData(nPersID) then begin
        ...
        MessageBox(Application.Handle,
          'Ошибка при обращении к базе данных!',
          'Печать листка-сообщения в ОВК',MB_OK or MB_ICONERROR);
        Result := -1;
      end
      else
        if not dmMain.PrintData(Checked,UseSend_Date, UseReason, Send_Date, Reason, chkLabel.Checked) then begin
          Result := -2;
          SaveEvent(dmMain.dbMain, evs_Report_Error, sEventObject,
