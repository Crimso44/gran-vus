  ...
  old_conn := dmMain.dbMain.ConnectionObject;
    try
      ...
      try
       if cbxOVK.Items.Count > 0 then begin
        ...
        MessageBox(Application.Handle,
          'Нет уволенных работников, подлежащих снятию со спецучета!',
          'Письма в ОВК о снятии со спецучета уволенных',MB_OK or MB_ICONWARNING);
         Exit;
       end;
      finally Free;
      end;
      ...
      if not dmMain.OpenData(nOrgID) then begin
        ...
        MessageBox(Application.Handle,
          'Ошибка при обращении к базе данных!',
          'Письма в ОВК о снятии со спецучета уволенных',MB_OK or MB_ICONERROR);
        Result := -1;
      end else
