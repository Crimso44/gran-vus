...
uses
  ...
  Db, dxTL, Vcl.Menus, Vcl.DBCtrls, Vcl.StdCtrls {$IFDEF DELPHI4}, Vcl.ImgList {$ENDIF},
  dxEdStr, dxCalc, dxCntner, dxEditor, dxExEdtr, dxDBEdtr, dxDBCtrl, dxUtils,
  dxGrDate, Vcl.Clipbrd, dxGrFCmn{$IFDEF DELPHI6}, Variants{$ENDIF};
...
begin
  Result := ATreeList.DataLink.Active and Assigned(Field) and
    ((InputChar = FormatSettings.ThousandSeparator) or Field.IsValidChar(InputChar));
end;
