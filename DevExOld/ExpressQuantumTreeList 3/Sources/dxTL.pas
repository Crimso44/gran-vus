...
uses
  SysUtils, Messages, Windows, Classes, Vcl.Graphics, Vcl.Menus, Vcl.Controls, Vcl.Forms,
  Vcl.StdCtrls, Vcl.Mask, CommCtrl {$IFDEF DELPHI4}, Vcl.ImgList{$ENDIF}, dxTLStr,
  Vcl.Clipbrd, dxCntner, dxEditor, Vcl.ComCtrls, dxUtils{$IFDEF DELPHI6}, Variants{$ENDIF};
...
begin
  ...
  begin
    // calc rect
    begin
      ARect.Left := (ARect.Left + ARect.Right - btSize) div 2;
      ARect.Top := YCenter - btSize div 2;
      ARect.Right := ARect.Left + btSize;
      ARect.Bottom := ARect.Top + btSize;
    end;
    // draw
    Windows.FrameRect(Handle, ARect, GetSysColorBrush(COLOR_GRAYTEXT)); {BTNSHADOW}
    begin
      Windows.FillRect(Handle, Rect(ARect.Left + 2, ARect.Top + 4, ARect.Right - 2{ARect.Left + 7}, ARect.Top + 4 + 1), {-}
        GetSysColorBrush(COLOR_WINDOWTEXT));
      if IsPlus then
        Windows.FillRect(Handle, Rect(ARect.Left + 4, ARect.Top + 2, ARect.Left + 4 + 1, ARect.Bottom - 2{ARect.Top + 7}),   {|}
          GetSysColorBrush(COLOR_WINDOWTEXT));
    end;
  end;
end;
...
begin
  begin
    ARect.Left := ((ARect.Left + ARect.Right) div 2) - (XWidth div 2);
    ARect.Right := ARect.Left + XWidth;
    ARect.Top := ((ARect.Top + ARect.Bottom) div 2) - (XHeight div 2);
    ARect.Bottom := ARect.Top + XHeight;
    if IsUp then
    begin
      // shadow
      PrevPen := SelectObject(DC, CreatePen(PS_SOLID, 1, GetSysColor(COLOR_BTNSHADOW)));
      Points[0] := Point(ARect.Left + XWidth div 2 - 1, ARect.Top);
      Points[1] := Point(ARect.Left, ARect.Bottom);
      Polyline(DC, Points, 2);
      Points[0] := Point(ARect.Left + XWidth div 2 - 2, ARect.Top + 1);
      Points[1] := Point(ARect.Left, ARect.Bottom - 1);
      Polyline(DC, Points, 2);
      // highlight
      DeleteObject(SelectObject(DC, CreatePen(PS_SOLID, 1, GetSysColor(COLOR_BTNHIGHLIGHT))));
      Points[0] := Point(ARect.Left + 1, ARect.Bottom - 1);
      Points[1] := Point(ARect.Left + XWidth - 1, ARect.Bottom - 1);
      Points[2] := Point(ARect.Left + XWidth div 2, ARect.Top - 1);
      Polyline(DC, Points, 3);
      Points[0] := Point(ARect.Left + XWidth div 2, ARect.Top + 1);
      Points[1] := Point(ARect.Left + XWidth - 1, ARect.Bottom - 1);
      Polyline(DC, Points, 2);
    ...
    begin
      // shadow
      PrevPen := SelectObject(DC, CreatePen(PS_SOLID, 1, GetSysColor(COLOR_BTNSHADOW)));
      Points[0] := Point(ARect.Left + XWidth - 1, ARect.Top);
      Points[1] := Point(ARect.Left, ARect.Top);
      Points[2] := Point(ARect.Left + XWidth div 2 - 1, ARect.Bottom);
      Polyline(DC, Points, 3);
      Points[0] := Point(ARect.Left + 1, ARect.Top + 1);
      Points[1] := Point(ARect.Left + XWidth div 2 - 1, ARect.Bottom - 1);
      Polyline(DC, Points, 2);
      // highlight
      DeleteObject(SelectObject(DC, CreatePen(PS_SOLID, 1, GetSysColor(COLOR_BTNHIGHLIGHT))));
      Points[0] := Point(ARect.Left + XWidth - 1, ARect.Top + 1);
      Points[1] := Point(ARect.Left + XWidth div 2, ARect.Bottom);
      Polyline(DC, Points, 2);
      Points[0] := Point(ARect.Left + XWidth - 2, ARect.Top + 1);
      Points[1] := Point(ARect.Left + XWidth div 2, ARect.Bottom - 1);
      Polyline(DC, Points, 2);
      DeleteObject(SelectObject(DC, PrevPen));
    end;
  end;
...
begin
  ...
  BitBlt(DC, R.Left, R.Top, R.Right-R.Left, R.Bottom-R.Top, 0, 0, 0, DSTINVERT);
end;
...
begin
  ...
  begin
    XSize := (ARect.Right - ARect.Left) div 2;
    if not Odd(XSize) then Inc(XSize);
    X := (ARect.Left + ARect.Right - XSize) div 2;
    Y := (ARect.Top + ARect.Bottom - XSize div 2) div 2 - Byte(Odd(ARect.Bottom - ARect.Top)){1};
    P[1] := Point(X, Y);
    P[2] := Point(X + XSize - 1, Y);
    P[3] := Point(X + XSize div 2, Y + XSize div 2);
...
begin
  ...
  begin
    begin
      if (ARect.Right - ARect.Left) >= dxGridHeaderFilterBoxWidth then
      begin
        R := Rect(ARect.Right - dxGridHeaderFilterBoxWidth, ARect.Top, ARect.Right, ARect.Bottom);
        Dec(ARect.Right, dxGridHeaderFilterBoxWidth);
        Inc(R.Bottom);
        // Combo Button
  ...
  if ASorted <> csNone then
    begin
      if (ARect.Right - ARect.Left) >= dxGridSortedShapeMinWidth then
      begin
        R := Rect(ARect.Right - dxGridSortedShapeMinWidth, ARect.Top, ARect.Right, ARect.Bottom);
        Dec(ARect.Right, dxGridSortedShapeMinWidth);
  ...
  if (AGlyph <> nil) and not AGlyph.Empty then
    begin
      ...
      begin
        R := Rect(ARect.Left - 1, ARect.Top - 1, ARect.Left + AGlyph.Width, ARect.Bottom + 2 - Byte(ADown and not AFlat));
        if AFlat then OffsetRect(R, -1 , 0);
        if R.Right > ARect.Right + Byte(not ADown or AFlat) + Byte(AFlat) then
          R.Right := ARect.Right + Byte(not ADown or AFlat) + Byte(AFlat);
        ARect.Left := R.Right;
  ...
  begin
    GetTextExtentPoint32(DC, PChar(AText), Length(AText), XSize);
    if XSize.CX <= (ARect.Right - ARect.Left) then
      Flag := DT_NOCLIP;
  end;
...
begin
  ...
  if (ALookAndFeel <> lfStandard) and ADown then
  BitBlt(bmpDC, R.Left, R.Top, R.Right-R.Left, R.Bottom-R.Top, 0, 0, 0, DSTINVERT);
  BitBlt(DC, AClipRect.Left, AClipRect.Top, AClipRect.Right-AClipRect.Left, AClipRect.Bottom-AClipRect.Top, bmpDC, AClipRect.Left - ARect.Left, AClipRect.Top - ARect.Top, SRCCOPY);
end;
...
begin
  ...
  begin
    with dxIndicatorImages do
      Draw(ABitmap.Canvas, (R.Left + R.Right - Width) shr 1,
        (R.Top + R.Bottom - Height) shr 1, Integer(AKind) - 1);
  end;
  BitBlt(DC, AClipRect.Left, AClipRect.Top, AClipRect.Right-AClipRect.Left, AClipRect.Bottom-AClipRect.Top, ABitmap.Canvas.Handle, AClipRect.Left - ARect.Left, AClipRect.Top - ARect.Top, SRCCOPY);
end;
...
begin
  ...
  begin
    ...
    SetRectEmpty(RRect);
    begin
      RRect := Rect(ARect.Right - TextXOffset, ARect.Top, ARect.Right, ARect.Bottom);
      if RRect.Left < ARect.Left then
        RRect.Left := ARect.Left;
      ARect.Right := RRect.Left;
      if AAlignment in [taRightJustify, taCenter] then
      begin
        LRect := Rect(ARect.Left, ARect.Top, ARect.Left + TextXOffset, ARect.Bottom);
    ...
    case AAlignment of
      ...
      taRightJustify:
        begin
          ...
          LeftOffset := (ARect.Right - ARect.Left) - SizeText.CX{ - TextXOffset};
        end;
     else {taCenter:}
        begin
          GetTextExtentPoint(DC, PChar(AText), Length(AText), SizeText);
  ...
  begin
    ...
    begin
      ...
      Draw(bmpDC, R);
      SelectObject(bmpDC, PrevFont);
      BitBlt(DC, ARect.Left, ARect.Top, ARect.Right-ARect.Left, ARect.Bottom-ARect.Top, bmpDC, 0, 0, SRCCOPY);
    end
    else
    begin
      Draw(DC, ARect);
...
begin
  begin
    InvertRect(DC, Rect(R.Left, R.Top, R.Right, R.Top + 1));
    InvertRect(DC, Rect(R.Right - 1, R.Top + 1, R.Right, R.Bottom));
    InvertRect(DC, Rect(R.Left, R.Top + 1, R.Left + 1, R.Bottom{ + 1}));
    InvertRect(DC, Rect(R.Left + 1, R.Bottom - 1, R.Right - 1, R.Bottom));
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      BandLeftCoord := BandRect.Left;
      BandRightCoord := BandRect.Right;
      if FixedLeft <> -1 then
      begin
        BandWidth := GetBandWidth(iStart) + GetIndentWidth; // Indent
        Inc(BandLeftCoord, BandWidth);
        FixedBandLeftRect := Rect(BandRect.Left, BandRect.Top, BandRect.Left + BandWidth, CellsRect.Top);
        if FNewItemRowHeight > 0 then
          Dec(FixedBandLeftRect.Bottom, FNewItemRowSeparatorHeight);
        BandsInfo^[BandCount].Index := iStart;
        BandsInfo^[BandCount].BandRect := Rect(BandRect.Left, BandRect.Top,
          FixedBandLeftRect.Right - FFixedBandLineWidth, BandRect.Bottom);
        BandsInfo^[BandCount].BandClipRect := BandsInfo^[BandCount].BandRect;
        FixedBandLeftIndex := iStart;
        Inc(iStart);
        Inc(BandCount);
      end;
      if (BandRect.Right - BandRect.Left){BandRect} > BandTotalWidth then // empty rect
      begin
        BandRect.Right := BandRect.Left + BandTotalWidth;
        HeaderRect.Right := BandRect.Right; // correct HeaderRect
        EmptyRectRight := Rect(BandRect.Right, BandRect.Top, CRect.Right, CellsRect.Bottom);
      ...
      begin
        ...
        FixedBandRightRect := Rect(BandRect.Right - BandWidth, BandRect.Top, BandRect.Right, CellsRect.Top);
        if FNewItemRowHeight > 0 then
      ...
      begin
        ...
        begin
          ...
          BandsInfo^[BandCount].BandRect := Rect(BandLeftCoord + BandCoord, BandRect.Top,
            BandLeftCoord + BandCoord + BandWidth, BandRect.Bottom);
          BandsInfo^[BandCount].BandClipRect := BandsInfo^[BandCount].BandRect;
      ...
      begin
        ...
        BandsInfo^[BandCount].BandRect := Rect(FixedBandRightRect.Left + FFixedBandLineWidth, BandRect.Top,
          FixedBandRightRect.Right, BandRect.Bottom);
        with BandsInfo^[BandCount].BandRect do
          BandsInfo^[BandCount].BandClipRect := Rect(BandRect.Left, BandRect.Top, BandRect.Right, BandRect.Bottom);
        Inc(BandCount);
      end;
    end;
    // headers info
    ...
    begin
        ...
        begin
          ...
          try
            ...
            begin
              ...
              XBandRect := BandsInfo^[i].BandRect;
              R := Rect(XBandRect.Left + HeaderLeftCoord, XBandRect.Bottom + FHeaderRowHeight*j,
                  XBandRect.Left + HeaderLeftCoord + W, XBandRect.Bottom + FHeaderRowHeight*j + H);
              ...
              with BandsInfo^[i] do
                ...
                begin
                  ...
                  begin
                    XBandRect := BandsInfo^[i].BandRect;
                    HeaderEmptyRect := Rect(HeaderClipRect.Left, XBandRect.Bottom + FHeaderRowHeight * (j + L),
                        HeaderClipRect.Right, XBandRect.Bottom + FHeaderRowHeight * rMaxCount);
                  end;
                end;
            end;
            // end fill array
      ...
      begin
        ...
        XBandRect := BandsInfo^[i].BandRect;
        R := Rect(XBandRect.Left, XBandRect.Bottom, XBandRect.Left + W, XBandRect.Bottom + FHeaderRowHeight * rMaxCount);
        with BandsInfo^[i] do
...
(*    if (Parent = nil) {not HandleAllocated} then
    ...
    GroupPanelRect := Rect(CRect.Left, CRect.Top, CRect.Right, CRect.Top + FGroupPanelHeight);
    // Band
    BandRect := Rect(CRect.Left + FIndicatorWidth, CRect.Top + FGroupPanelHeight,
        CRect.Right, CRect.Top + FGroupPanelHeight + FBandHeight);
    if FIndicatorWidth > 0 then // band button
    BandButtonRect := Rect(BandRect.Left - FIndicatorWidth, BandRect.Top, BandRect.Left, BandRect.Bottom);
    // Header
    HeaderRect := Rect(CRect.Left + FIndicatorWidth, CRect.Top + FGroupPanelHeight + FBandHeight,
        CRect.Right, CRect.Top + FGroupPanelHeight + FBandHeight + FHeaderHeight);
    if FIndicatorWidth > 0 then // header button
    HeaderButtonRect := Rect(HeaderRect.Left - FIndicatorWidth, HeaderRect.Top, HeaderRect.Left, HeaderRect.Bottom);
    // New Item Row Height
    if (FNewItemRowHeight > 0) then
    NewItemRowRect := Rect(CRect.Left, CRect.Top + FGroupPanelHeight + FBandHeight + FHeaderHeight,
        CRect.Right, CRect.Top + FGroupPanelHeight + FBandHeight + FHeaderHeight + FNewItemRowHeight);
    // Cells
    CellsRect := Rect(CRect.Left + FIndicatorWidth,
        CRect.Top + FGroupPanelHeight + FBandHeight + FHeaderHeight + FNewItemRowHeight,
        CRect.Right, CRect.Bottom - FFooterHeight - FStatusHeight);
    if CellsRect.Top > CellsRect.Bottom then CellsRect.Bottom := CellsRect.Top;
    // Indicator
    IndicatorRect := Rect(CRect.Left, CellsRect.Top, CellsRect.Left, CellsRect.Bottom);
    // Footer
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            XHeaderClipRect := HeadersInfo^[i].HeaderClipRect;
              RClip := Rect(XHeaderClipRect.Left, R.Top, XHeaderClipRect.Right, R.Bottom);
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              RClip := Rect(HeaderClipRect.Left, R.Top, HeaderClipRect.Right, R.Bottom);
            end;
            if PtInRect(R, Pos) and PtInRect(RClip, Pos) then
            begin
              Result.Band := HeadersInfo^[i].BandIndex;
...
var
  ...
  begin
    ...
    begin
      ...
      FillRect(ADC, ARect, ABrush);
      if ADown then
        BitBlt(ADC, ARect.Left, ARect.Top, ARect.Right - ARect.Left, ARect.Bottom - ARect.Top, 0, 0, 0, DSTINVERT);
    end
    else
...
//          LookAndFeel{GetLookAndFeelStyle});
      ...
      begin
        R := Rect(FixedBandLeftRect.Right - FFixedBandLineWidth, FixedBandLeftRect.Top, FixedBandLeftRect.Right, FixedBandLeftRect.Bottom);
      ...
      begin
        R := Rect(FixedBandRightRect.Left, FixedBandRightRect.Top, FixedBandRightRect.Left + FFixedBandLineWidth, FixedBandRightRect.Bottom);
        Windows.FillRect(Handle, R, FixedBandLineBrush{GetSysColorBrush(COLOR_WINDOWFRAME)});
      end;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        ExcludeClipRect(Handle, IndicatorRect.Left, IndicatorRect.Top, IndicatorRect.Right, IndicatorRect.Bottom);
      end;
    end;
  end;
...
//                Windows.FrameRect(Canvas.Handle, R, GetSysColorBrush(COLOR_BTNSHADOW));
        ...
        ExcludeClipRect(Handle, FooterRect.Left, FooterRect.Top, FooterRect.Right, FooterRect.bottom);
      end;
    end;
  end;
  ...
  procedure DrawStatus;
    ...
    begin
      ...
      begin
        ...
        LineTo(ADC, ARect.Right - 2, ARect.Bottom - 1);
        LineTo(ADC, ARect.Right - 1, ARect.Bottom - 1);
        LineTo(ADC, ARect.Left + 1, ARect.Top);
        LineTo(ADC, ARect.Left, ARect.Top);
        { / }
        MoveToEx(ADC, ARect.Right - 1, ARect.Top, nil);
        LineTo(ADC, ARect.Left + 1, ARect.Bottom - 1);
        LineTo(ADC, ARect.Left, ARect.Bottom - 1);
        LineTo(ADC, ARect.Right - 2, ARect.Top);
  ...
  begin
    with Canvas, DrawInfo do
      ...
      begin
        ...
        if AShowStatusButton then
        begin
          R := Rect(StatusRect.Left, StatusRect.Top, StatusRect.Left + dxGridStatusCloseButtonSizeX, StatusRect.Top + dxGridStatusCloseButtonSizeY);
        ...
        ExcludeClipRect(Handle, StatusRect.Left, StatusRect.Top, StatusRect.Right, StatusRect.bottom);
      end;
  end;
            ...
            begin
              ...
              try
                ...
                begin
                  ...
                  if not IsRectEmptyEx(FixedBandLeftRect) then
                  begin
                    R := Rect(FixedBandLeftRect.Right - FFixedBandLineWidth, R.Top, FixedBandLeftRect.Right, R.Bottom);
                    Windows.FillRect(Handle, R, FixedBandLineBrush{GetSysColorBrush(COLOR_WINDOWFRAME)});
                  end;
                  if not IsRectEmptyEx(FixedBandRightRect) then
                  begin
                    R := Rect(FixedBandRightRect.Left, R.Top, FixedBandRightRect.Left + FFixedBandLineWidth, R.Bottom);
                    Windows.FillRect(Handle, R, FixedBandLineBrush{GetSysColorBrush(COLOR_WINDOWFRAME)});
                  end;
                  Inc(R.Top, CorrectY);
...
begin
  ...
  begin
    Result := GetHorzGridRect.Right - GetHorzGridRect.Left;
    // Check MinWidth
    W := GetIndentWidth;
    FlagNotSizing := True;
    for I := 0 to GetBandCount - 1 do
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          XHeaderRect := HeadersInfo^[i].HeaderRect;
          begin
            Start := XHeaderRect.Left;
            Min := XHeaderRect.Left + GetHeaderMinWidth(AbsoluteIndex) +
...
begin
  ...
  with SaveArrowsBitmap Do
  begin
    Width := Max(Width, FArrowsRect.Right - FArrowsRect.Left);
    Height := Max(Height, FArrowsRect.Bottom - FArrowsRect.Top);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if CurNode = Node then
      begin
        Result := Rect(CellsRect.Left, Y, CellsRect.Right, Y + H);
        if not IsRectEmpty(IndicatorRect) then
...
begin
  ...
  if not IsRectEmpty(DI.StatusRect) then
    begin
      Result := Rect(DI.StatusRect.Left, DI.StatusRect.Top, DI.StatusRect.Left + dxGridStatusCloseButtonSizeX, DI.StatusRect.Top + dxGridStatusCloseButtonSizeY);
      OffsetRect(Result, 3, (DI.StatusRect.Bottom - DI.StatusRect.Top - dxGridStatusCloseButtonSizeY) div 2);
    end;
end;
...
begin
  ...
  begin
    ...
    // Calc RowCount
    CalcRowCount(TopVisibleNode, R.Bottom - R.Top, ARowCount, Dummy);
    // Check ClientHeight and ClientWidht
    if (GetAbsoluteCount <= ARowCount) and
      (GetScrollableBandWidth <= (R.Right - R.Left)) then // hide Scroll Bars
    begin
...
begin
  CalcRectInfo(DrawInfo);
  CalcRowCount(TopVisibleNode, DrawInfo.CellsRect.Bottom - DrawInfo.CellsRect.Top, FRowCount, FVisibleRowCount);
    ...
    try
      ...
      begin
        ...
        begin
          ...
          Cell_Rect := Rect(0, 0, ARect.Right - ARect.Left, ARect.Bottom - ARect.Top);
...
begin
  ...
  begin
    ...
    CalcRectInfo(DrawInfo);
    CalcRowCount(GetAbsoluteNode(AIndex), DrawInfo.CellsRect.Bottom - DrawInfo.CellsRect.Top, FRealRowCount, FDummy);
...
begin
  ...
  begin
    BitBlt(SaveArrowsBitmap.Canvas.Handle, 0, 0, FSaveArrowsRect.Right-FSaveArrowsRect.Left, FSaveArrowsRect.Bottom-FSaveArrowsRect.Top,
    ...
    begin
      P[0] := Point(FSaveArrowsRect.Left + 5, FSaveArrowsRect.Top + 8);
      P[1] := Point(FSaveArrowsRect.Left, FSaveArrowsRect.Top + 3);
      P[2] := Point(FSaveArrowsRect.Left + 3, FSaveArrowsRect.Top + 3);
      P[3] := Point(FSaveArrowsRect.Left + 3, FSaveArrowsRect.Top);
      P[4] := Point(FSaveArrowsRect.Left + 7, FSaveArrowsRect.Top);
      P[5] := Point(FSaveArrowsRect.Left + 7, FSaveArrowsRect.Top + 3);
      P[6] := Point(FSaveArrowsRect.Left + 10, FSaveArrowsRect.Top + 3);
      Polygon(DC, P, 7);
      P[0] := Point(FSaveArrowsRect.Left + 5, FSaveArrowsRect.Bottom - 9);
      P[1] := Point(FSaveArrowsRect.Left + 10, FSaveArrowsRect.Bottom - 4);
      P[2] := Point(FSaveArrowsRect.Left + 7, FSaveArrowsRect.Bottom - 4);
      P[3] := Point(FSaveArrowsRect.Left + 7, FSaveArrowsRect.Bottom - 1);
      P[4] := Point(FSaveArrowsRect.Left + 3, FSaveArrowsRect.Bottom - 1);
      P[5] := Point(FSaveArrowsRect.Left + 3, FSaveArrowsRect.Bottom - 4);
      P[6] := Point(FSaveArrowsRect.Left, FSaveArrowsRect.Bottom - 4);
      Polygon(DC, P, 7);
    end;
  ...
  begin
    BitBlt(DC, FSaveArrowsRect.Left, FSaveArrowsRect.Top, FSaveArrowsRect.Right - FSaveArrowsRect.Left, FSaveArrowsRect.Bottom - FSaveArrowsRect.Top,
...
begin
  ...
  begin
    ...
    InflateRect(R, -1, -1);
    R := Rect(R.Right-W, R.Bottom-H, R.Right, R.Bottom);
...
var
  B: Byte;
  I, L, XSize, ItemCount: Integer;
  Info: PdxTextNodeInfo;
  S: string;
begin
  Stream.ReadBuffer(XSize, SizeOf(XSize));
  GetMem(Info, XSize);
  try
    Stream.ReadBuffer(Info^, XSize);
...
var
  I, XSize, L, ItemCount: Integer;
  Info: PdxTextNodeInfo;
  S: string;
begin
  XSize := SizeOf(TdxTextNodeInfo);
  GetMem(Info, XSize);
  try
    ...
    Stream.WriteBuffer(XSize, SizeOf(XSize));
    Stream.WriteBuffer(Info^, XSize);
    for I := 0 to Info^.StrCount - 1 do
    begin
      S := FStrings[I];
      L := Length(S);
...
begin
  begin
    Windows.FillRect(DC, Rect(ARect.Left, ARect.Top, ARect.Right, ARect.Top + 1), ABrush);
    Windows.FillRect(DC, Rect(ARect.Left, ARect.Bottom - 1, ARect.Right, ARect.Bottom), ABrush);
    InflateRect(ARect, 0, -1);
    if ALeftEdge then
    begin
      Windows.FillRect(DC, Rect(ARect.Left, ARect.Top, ARect.Left + 1, ARect.Bottom), ABrush);
      Inc(ARect.Left, 1);
    end;
    if ARightEdge then
    begin
      Windows.FillRect(DC, Rect(ARect.Right - 1, ARect.Top, ARect.Right, ARect.Bottom), ABrush);
      Dec(ARect.Right, 1);
    end;
  end;
end;
  ...
  try
    ...
    try
      ...
      try
        ...
        System.Move(Pointer(FList.List)^, Pointer(FSelectedNodes.List)^, FList.Count * SizeOf(Pointer));
      finally
