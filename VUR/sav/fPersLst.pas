...
type
  TfmPersonList = class(TForm)
    ...
    dbgDataColumn188: TdxDBGridMaskColumn;
...
uses fPersonCard, msg, misc, MD5, fFltMgr, fRegister, DateUtils, BirthDay;
...
procedure TfmPersonList.actDelExecute(Sender: TObject);
  ...
  begin
    with TADOQuery.Create(Self) do begin
      Connection := dmMain.dbMain;
      SQL.Text := 'DELETE FROM '+tbl_name+' WHERE PERS_ID='+
...
//     (ANode.Strings[dbgDataColumnWDISCL.Index]<>'')
  ...
  if ANode.AbsoluteIndex = dbgData.FocusedNode.AbsoluteIndex
  then begin
    ...
    if qrDataReserved.AsInteger = 1 then begin
      ...
      if qrDataDefPost        .AsInteger = 0 then begin
        ...
        if qrDataDefPost_WSost.AsInteger = 0 then
          Msg := Msg + #13#10'Офицеры такого профиля не фигурируют в ПДП.';
      end else
