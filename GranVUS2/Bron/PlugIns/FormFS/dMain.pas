...
const
  sEventObject = 'Распределение организаций по видам собственности';
...
begin
  EkRTF1.Infile := tlPath + 'FormFS.rtf';
  ...
  try
    try
      ...
      with TADOQuery.Create(Self) do try
        ...
        Close;
        SQL.Text := 'SELECT'#10 +
                    '  SUM_CITY  = SUM(CITY)'#10 +
                    ', SUM_TOWN     = SUM(TOWN)'#10 +
                    ', SUM_VILLAGE   = SUM(VILLAGE)'#10 +
                    ', SUM_TOWNVILLAGE  = SUM(TOWNVILLAGE)'#10 +
                    ', SUM_INCITY     = SUM(INCITY)'#10 +
                    'FROM FFS2010'#10 +
                    'WHERE SUBJ_ID = ' + IntToStr(ID);
        Open;
        EkRTF1.CreateVar('SUM_ALL', FieldByName('SUM_CITY').AsInteger +
          FieldByName('SUM_TOWN').AsInteger + FieldByName('SUM_VILLAGE').AsInteger +
          FieldByName('SUM_TOWNVILLAGE').AsInteger + FieldByName('SUM_INCITY').AsInteger);
        EkRTF1.CreateVar('SUM_CITY', FieldByName('SUM_CITY').AsInteger);
        EkRTF1.CreateVar('SUM_ALLTOWNVILLAGE', FieldByName('SUM_TOWN').AsInteger +
          FieldByName('SUM_VILLAGE').AsInteger + FieldByName('SUM_TOWNVILLAGE').AsInteger);
        EkRTF1.CreateVar('SUM_TOWN', FieldByName('SUM_TOWN').AsInteger);
        EkRTF1.CreateVar('SUM_VILLAGE', FieldByName('SUM_VILLAGE').AsInteger);
        EkRTF1.CreateVar('SUM_TOWNVILLAGE', FieldByName('SUM_TOWNVILLAGE').AsInteger);
        EkRTF1.CreateVar('SUM_INCITY', FieldByName('SUM_INCITY').AsInteger);
        Close;

        SQL.Text :=
          'SELECT FIO FROM SUBJ_INFO Where Subj_Info_Id = 2 and SUBJ_ID = '+IntToStr(ID);
        Open;
        EkRTF1.CreateVar('BRON_NAME', FieldByName('FIO').AsString);
...
begin
  case ABS(DataSet.RecNo) of
    1: DataSet.FieldByName('N').AsString := '1.';
    2: DataSet.FieldByName('N').AsString := '    а)';
    else DataSet.FieldByName('N').AsString := IntToStr(ABS(DataSet.RecNo)-1)+'.';
  end;
