...
begin
  ...
  try
    ...
    SQL.Text :=
      ...
      ' ((select COUNT(*) from PERS_SET)=0 or P.PERS_ID in (select PERS_ID from PERS_SET)) ' +
      'AND P.Wuchet2_IsWork = 1 and ' +
      '(((select max(In_Date) from Appointment where pers_id = P.pers_id and post_id in (select post_id from kpost where cprof_id <> 500)) IS NOT NULL AND P.OUT_ORD_DATE IS NOT NULL) ' +
      'OR ((select max(In_Date) from Appointment where pers_id = P.pers_id and post_id in (select post_id from kpost where cprof_id = 500)) IS NOT NULL AND P.ORDERDISMISSAL_DATE IS NOT NULL)) ' +
      'ORDER BY OVK_NAME';
    Open;
    cbxOVK.Items.Clear;
