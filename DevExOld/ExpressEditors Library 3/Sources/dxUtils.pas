...
uses
  ...
  Vcl.StdCtrls, CommCtrl{$IFDEF DELPHI4}, Vcl.ImgList {$ENDIF};
...
begin
  begin
    X := R.Left - FullRect.Left;
    Y := R.Top - FullRect.Top;
    BW := FullRect.Right - FullRect.Left;
    BH := FullRect.Bottom - FullRect.Top;
    ...
    try
      ...
      BitBlt(DrawDC, FullRect.Left, FullRect.Top, FullRect.Right - FullRect.Left, FullRect.Bottom - FullRect.Top, DC, 0, 0, SRCCOPY);
    finally
      DeleteObject(SelectObject(DC, B));
      DeleteDC(DC);
    end;
    ...
    if ClipFlag then
      begin
        if Format and DX_DTR_TRANSPARENT = 0 then // not Transparent
          FillRect(DC, Rect(IndentRect.Right, FullRect.Top, FullRect.Right, FullRect.Bottom), Brush);
        FullRect.Right := IndentRect.Right;
      end;
    begin
      ...
      begin
        case Format and DX_DTR_ALIGNMENT_MASK of
          ...
          DX_DTR_CENTER:
            TextRect.Left := (TextRect.Left + TextRect.Right - Size.CX) shr 1;
        end;
      ...
      begin
        TextRect.Top := (TextRect.Top + TextRect.Bottom - Size.CY) shr 1;
