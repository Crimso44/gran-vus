  ...
  old_conn := dmMain.dbMain.ConnectionObject;
    try
      ...
      try
       if cbxOVK.Items.Count > 0 then begin
        ...
        MessageBox(Application.Handle,
          'Нет работников, подлежащих постановке на спецучет!',
          'Письма в ОВК о постановке на спецучет',MB_OK or MB_ICONWARNING);
         Exit;
       end;
      finally Free;
      end;
      ...
      if not dmMain.OpenData(nOrgID) then begin
        ...
        MessageBox(Application.Handle,
          'Ошибка при обращении к базе данных!',
          'Письма в ОВК о постановке на спецучет',MB_OK or MB_ICONERROR);
        Result := -1;
      end else
