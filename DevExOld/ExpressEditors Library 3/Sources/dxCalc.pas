...
uses
  ...
  Vcl.StdCtrls, Vcl.Clipbrd, dxCntner;
...
begin
  ...
  begin
    ...
    if i = cbDecimal then FButtons[i].Text := FormatSettings.DecimalSeparator
...
//  #27 : Result := cbClear;
  '@' : Result := cbSqrt;
    ...
    if FormatSettings.DecimalSeparator = Ch then Result := cbDecimal;
  end;
end;
  ...
  if Clipboard.HasFormat(CF_TEXT) then
    try
      ...
      repeat
        ...
        begin
          S1 := S1 + Copy(S, 1, i - 1);
          S := Copy(S, i + Length(FormatSettings.CurrencyString), MaxInt);
        end
        else
          S1 := S1 + S;
      until i <= 0;
...
begin
  ...
  begin
    CheckFirst;
    if Pos(FormatSettings.DecimalSeparator, EditValue) = 0 then
      SetEditValue(EditValue + FormatSettings.DecimalSeparator);
    Exit;
  end;
  case ButtonKind of
    cbRev:
