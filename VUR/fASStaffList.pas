...
const
  ...
  PagesTabs: array [0..PagesCount-2] of String =
   ('Штатная численность',
    'Работает',
    'Некомплект(+)/избыток(-) бойцов в АСП');
...
procedure Go;
const MyCaption: String = 'Штатная численность Аварийно-Спасательных Подразделений';
begin
...
begin
  if AColumn.Tag > 0 then
    AFont.Style := []
  else if StartsText('(штаб)',AColumn.Caption) then
