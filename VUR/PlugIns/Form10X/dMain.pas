...
const
  sEventObject = 'Форма № 10 (Личная карточка)';
...
function ShortDATE(ADate: TDateTime): String;
var year,month,day: Word;
begin
  if ADate=0 then Result := '' else begin
    DecodeDate(ADate,year,month,day);
...
begin
  ...
  EkRTF1.Infile := GetTemplatesDir + 'form10x.rtf';
  EkRTF1.Outfile := GetReportsDir + 'Личная карточка.rtf';
end;
...
//      else AppointmentRowCount := 8;
    ...
    with taAppointment do begin
      ...
      if (not Worker) and not qrPers.FieldByName('Dismissal_Date').IsNull then begin
        ...
        Append;
        FieldByName('IN_ORD').AsString := 'Приказ № ' +
          qrPers.FieldByName('OrderDismissal').AsString + ' от ' + ConfDate(qrPers.FieldByName('OrderDismissal_Date').AsDateTime);
        FieldByName('POST_NAME').AsString := 'ОТЧИСЛЕН';
        FieldByName('XOVK_ID').Value := qrOVK.FieldByName('OVK_ID').Value;
...
begin
  try
    ...
    while not qrFamily.EOF do begin
      dxFamily.Append;
      if cnt = 0 then
        dxFamily.FieldByName('Title').Value := '7. Состав семьи';
      dxFamily.FieldByName('FamType1').Value :=
        qrFamily.FieldByName('FT_NAME').AsString;
      dxFamily.FieldByName('FamName1').Value :=
        qrFamily.FieldByName('FAM_NAME').AsString;
      dxFamily.FieldByName('FamYear1').Value :=
    ...
    EkRTF1.Infile := ExtractFilePath(EkRTF1.Infile)+'form10x.rtf';
    EkRTF1.Outfile := ExtractFilePath(EkRTF1.Outfile)+
      'Форма № 10 (Личная карточка) '+
